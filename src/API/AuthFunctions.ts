...
async function LoginFunction (username:string, password:string, setError: (error:string) => void) {
    fetch('https://to-do-server-w0ax.onrender.com/auth/login', {
        method: 'POST',
        headers: {
...
async function RegisterFunction(username:string, password:string, setError: (error:string) => void, navigate:NavigateFunction) {
    fetch('https://to-do-server-w0ax.onrender.com/auth/register', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
        },
...
async function CheckLogin(): Promise<boolean> {
    try {
        const response = await fetch('https://to-do-server-w0ax.onrender.com/auth/loginCheck', {
            method: 'POST',
